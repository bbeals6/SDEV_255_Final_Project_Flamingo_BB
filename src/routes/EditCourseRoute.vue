...
export default {
  ...
  async created() {
    ...
    try {
      this.course = (await axios.get(`http://localhost:3000/course/${this.$route.params.id}`, config)).data;
  ...
  methods: {
    async submitForm() {
      try {
        await axios.post(`http://localhost:3000/course/update/${this.$route.params.id}`, this.course, this.config)
